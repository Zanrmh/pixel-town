...
import { Player } from '../../convex/pixelTown/player';
import { Conversation } from '../../convex/pixelTown/conversation';
// Thành phần này hiển thị các tin nhắn trong một cuộc trò chuyện, bao gồm tin nhắn văn bản, thông báo trạng thái tham gia/rời khỏi của người chơi, và dấu hiệu "đang nhập".
// Nó cũng cung cấp chức năng nhập tin nhắn mới nếu người chơi đang tham gia cuộc trò chuyện.
  ...
  if (conversation.kind === 'active') {
          ...
          node: (
            <div key={`joined-${playerId}`} className="leading-tight mb-6">
              <p className="text-brown-700 text-center">
                {playerName} đã tham gia cuộc trò chuyện.
              </p>
            </div>
          ),
          time: started,
    ...
    for (const playerId of conversation.doc.participants) {
      ...
      membershipNodes.push({
        node: (
          <div key={`joined-${playerId}`} className="leading-tight mb-6">
            <p className="text-brown-700 text-center">{playerName} đã tham gia cuộc trò chuyện.</p>
          </div>
        ),
        time: started,
      });
      ...
      membershipNodes.push({
        node: (
          <div key={`left-${playerId}`} className="leading-tight mb-6">
            <p className="text-brown-700 text-center">{playerName} đã rời cuộc trò chuyện.</p>
          </div>
        ),
